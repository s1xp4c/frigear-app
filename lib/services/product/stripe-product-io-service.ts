...
const transformInternalProductToStripe = (product: Product): Stripe.ProductCreateParams => {
    const {
        name,
        description,
        active,
    ...
    return {
        ...
        marketing_features: features ? Object.keys(features).map(feature => ({
...
export default class StripeProductIOService {
    ...
    async importProductsFromStripe(limit?: number): Promise<Product[]> {
        ...
        return await Promise.all(products.map(async (stripeProduct) => {
            const attributes = transformStripeProduct({object: stripeProduct});
            attributes.slug = uuidv4();
            if (stripeProduct && stripeProduct.id) {
                const existing = await this.productService.getByStripeId(stripeProduct.id);
                if (!existing) {
    ...
    async exportProductsToStripe(limit?: number): Promise<Stripe.Product[]> {
        ...
        return await Promise.all(products.slice(0, limit).map(async (product) => {
            return await this.stripeProductService.create(transformInternalProductToStripe(product));
        }));
    }
}