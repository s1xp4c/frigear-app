...
describe('service/webhooks/stripe-webhook-handler', () => {
    ...
    it('should throw on unhandled events', async () => {
        ...
        expect(async ()=>await handler.handleEvent(buildStripeEvent(eventName, stripeProduct)))
            .rejects.toThrow(`Unhandled event: ${eventName}`);
    });

    it('should create a new product', async () => {
        await handler.handleEvent(buildStripeEvent('product.created', stripeProduct));
    ...
    it('should update an existing product', async () => {
        ...
        await handler.handleEvent(buildStripeEvent('product.updated', {...stripeProduct, name}));
        const [product] = await service.all();
    ...
    it('should mark a product as inactive when deleted in stripe', async () => {
        ...
        await handler.handleEvent(buildStripeEvent('product.deleted', stripeProduct));
