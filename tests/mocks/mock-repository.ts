...
> implements IRepository<Key, Entity, Create, Update>
{
  ...
  async getById(id: Key): Promise<Entity> {
    const entity = this.state.find(
      (row) => row[this.mapping.id as never] === id,
    );
    if (!entity) {
  ...
  async getBySlug(slug: string): Promise<Entity> {
    const entity = this.state.find(
      (row) => row[this.mapping.slug as never] === slug,
    );
    if (!entity) {
      throw new NotFoundError();
    }
  ...
  async getBySecondaryId(stripeId: string): Promise<Entity> {
    const product = this.state.find(
      (row) => row[this.mapping.secondary_id as never] === stripeId,
    );
    if (!product) {
      throw new NotFoundError();
    }
